            ...
            <div className="flex gap-3">
              <div className="relative flex-1">
                <input
                  ...
                  placeholder="Search by name, barcode, QR code, serial number, asset number, manufacturer..."
                  className="w-full pl-10 pr-4 py-3 border border-gray-600 rounded-lg bg-gray-900 text-brand-primary-text placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                  onKeyDown={(e) => {
                    if (e.key === 'Enter') {
                      handleFiltersApply()
                    }
              ...
              <button
                ...
                className={`px-4 py-3 rounded-lg text-sm font-medium flex items-center gap-2 transition-colors ${
                  filtersExpanded || Object.values(filters).filter(v => v && v !== filters.search).some(v => v !== '')
                    ? 'bg-blue-600 hover:bg-blue-700 text-white'
                    : 'bg-white/5 hover:bg-white/10 border border-gray-600 text-gray-200'
                }`}
              >
                ...
                <span className="hidden sm:inline">
                  {Object.values(filters).filter(v => v && v !== filters.search).some(v => v !== '') ? 'Filters' : 'More Filters'}
                </span>
              </button>
            </div>
          </div>
